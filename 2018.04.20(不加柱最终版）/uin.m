function U=uin(r,theta,n,rho1,rho2,rho3,a,b,omega,c1,cl2,ct2,c3,rp,rhop,cp)
i=sqrt(-1);
k3=omega/c3;
syms x1  x2
...
u=(1/(rho3*omega^2))*i^(n)*epsilon*k3^(2)*qn(n,rho1,rho2,rho3,a,b,omega,c1,cl2,ct2,c3)*d2besselj(k3.*r).*cos(n.*theta)*2.*((1/(rho3*omega^2))*i^(n)*epsilon*k3*qn(n,rho1,rho2,rho3,a,b,omega,c1,cl2,ct2,c3)*dbesselj(k3.*r).*cos(n.*theta));
...
U=-2*pi*rp^(3)*rho3*(p3*(f1/(3*rho3^(2)*c3^(2)))-u*(f2/2));
end
