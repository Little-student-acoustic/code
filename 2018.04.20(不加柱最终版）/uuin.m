function U=uuin(r,theta,n,rho1,rho2,rho3,a,b,omega,c1,cl2,ct2,c3,rp,rhop,cp)
i=sqrt(-1);
k3=omega/c3;
syms x1  x2
...
%U=-2*pi*rp^(3)*rho3*(p3*(f1/3*rho3^(2)*c3^(2))-u*(f2/2))*2.*r.*cos(theta)-2*pi*rp^(3)*rho3*(pp3*(f1/3*rho3^(2)*c3^(2))-uu*(f2/2)).*(-r.*sin(theta)./(r.*cos(theta).^(2)+r.*sin(theta).^(2)));
U=-2*pi*rp^(3)*rho3*(p3*(f1/(3*rho3^(2)*c3^(2)))-u*(f2/2)).*(1./cos(theta))-2*pi*rp^(3)*rho3*(pp3*(f1/(3*rho3^(2)*c3^(2)))-uu*(f2/2)).*(-1./(sin(theta).*r));
end
